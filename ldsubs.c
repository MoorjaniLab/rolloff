...
// p4 is probability of chromosome if unlinked
   ...
   if (ychi <= 0.0) return 0.0 ;
   zscore = sqrt(ychi) ;
   if (p4[0] < p4t[0]) zscore *= -1.0 ;
   return zscore ;
}
...
*/
 return corrpt -> v12; // note the ztrans doesn't actually play a role now
...
// mode&2 => do NOT divide out by v22
{

  double y, yn, m1, m2, v11, v12, v22, r ;

  corrpt -> corr = corrpt -> Z = 0.0 ;
  if (corrpt -> S0 < 0.5) return -1 ;
  ...
  corrpt -> v11 = v11 = (corrpt -> S11 - yn * m1 * m1) / (yn-1) ; // modified from yn
  corrpt -> v12 = v12 = (corrpt -> S12 - yn * m1 * m2) / (yn-1) ;
  corrpt -> v22 = v22 = (corrpt -> S22 - yn * m2 * m2) / (yn-1) ;

  if (mode&2)
    y = corrpt -> corr = v12 / (v11 + 1.0e-20)  ;
  else
