...
#define WVERSION   "625a" 
/* Method used in Moorjani et al. 2013 which computes weighted covariance for dating admixture. This differs from the original implementation in Moorjani et al. 2011 PlosGen where weighted ancestry correlation was used.
*/
...
int main(int argc, char **argv)
{
  ...
  printf("Computes:\n");
  printnl();
  printf("sum( cov(X,Y) * delta(x)delta(y) ) / sum(delta(x)^2 delta(y)^2)\n");
  printnl();
  printf("where the sums are over pairs of sites (x,y) in bin d, cov(X,Y) is\n");
  printf("the sample covariance between random variables X and Y corresponding\n");
  printf("to 0, 1, or 2 allele counts at sites x and y, and delta(x), delta(y)\n");
  printf("are the allele frequency differences between ancestral populations\n");
  printf("which we use as weights.");
  printnl();
  printf("The resulting curve should be 2*alpha*beta*exp(-nd).\n");
  printnl();
  printnl();
